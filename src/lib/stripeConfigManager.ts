...
class StripeConfigManager {
  ...
  private loadAutomationConfig(): StripeAutomationConfig {
    return {
      ...
      enableSubscriptionManagement: process.env.STRIPE_SUBSCRIPTION_MANAGEMENT !== 'false'
    };
  }
  ...
  public async validateConfiguration(): Promise<StripeValidationResult> {
    ...
    if (!this.config.secretKey) {
      ...
      if (!this.config.secretKey.startsWith(expectedPrefix)) {
        result.warnings.push(`Secret key should start with ${expectedPrefix} for ${this.config.testMode ? 'test' : 'live'} mode`);
      }
    }

    // Validate publishable key
    if (!this.config.publishableKey) {
      result.errors.push('NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY is not configured');
      result.isValid = false;
    } else if (this.config.publishableKey === 'your_stripe_publishable_key_here') {
      result.errors.push('NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY contains placeholder value');
      result.isValid = false;
    } else {
      ...
      if (!this.config.publishableKey.startsWith(expectedPrefix)) {
        result.warnings.push(`Publishable key should start with ${expectedPrefix} for ${this.config.testMode ? 'test' : 'live'} mode`);
      }
    }

    // Validate webhook secret
    if (!this.config.webhookSecret) {
      result.warnings.push('STRIPE_WEBHOOK_SECRET is not configured - webhooks will not work');
    } else if (this.config.webhookSecret.startsWith('whsec_')) {
      result.configStatus.webhookSecret = true;
    } else {
      result.warnings.push('STRIPE_WEBHOOK_SECRET format appears invalid (should start with whsec_)');
    }
    ...
    if (this.stripe && result.configStatus.secretKey) {
      try {
        ...
        result.errors.push(`Stripe API connection failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        result.isValid = false;
      }
    }
    ...
    if (this.config.testMode && this.config.environment === 'production') {
      result.suggestions.push('Consider disabling test mode for production environment');
    }

    if (!this.config.testMode && this.config.environment === 'development') {
      result.suggestions.push('Consider enabling test mode for development environment');
    }
  ...
  public async updateConfiguration(updates: Partial<StripeEnvironmentConfig>): Promise<void> {
  ...
  public async testWebhookEndpoint(webhookUrl: string): Promise<{ success: boolean; error?: string }> {
    if (!this.stripe) {
