...
function generateMockDonations(count = 100): Donation[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    donations.push({
      ...
      source: ["website", "email_campaign", "social_media", "event"][
      ...
      processorFee: amount * 0.029 + 0.3, // Typical payment processor fee
      netAmount: amount - (amount * 0.029 + 0.3),
      isRecurring,
    ...
    milestones: [
      {
        ...
        reached: true,
        reachedDate: new Date("2024-03-15").toISOString(),
      },
      {
        amount: 250000,
        description: "Mid-year Target",
        reached: true,
        reachedDate: new Date("2024-06-20").toISOString(),
      },
...
class DonationStorageService {
  ...
  private initializeDefaults() {
    ...
    if (!this.getFromStorage(STORAGE_KEYS.DONATIONS)) {
      const donations = generateMockDonations(150);
      this.setToStorage(STORAGE_KEYS.DONATIONS, JSON.stringify(donations));

      // Generate donors based on donations
      const donors = generateMockDonors(donations);
      this.setToStorage(STORAGE_KEYS.DONORS, JSON.stringify(donors));
    }
    ...
    if (!this.getFromStorage(STORAGE_KEYS.GOALS)) {
      this.setToStorage(STORAGE_KEYS.GOALS, JSON.stringify(DEFAULT_GOALS));
    }

    if (!this.getFromStorage(STORAGE_KEYS.RECURRING)) {
      this.setToStorage(STORAGE_KEYS.RECURRING, JSON.stringify([]));
    }
  }
  ...
  async getAllDonors(): Promise<Donor[]> {
    return JSON.parse(this.getFromStorage(STORAGE_KEYS.DONORS, "[]"));
  }
  ...
  private async updateDonorStats(donorId: string): Promise<void> {
    ...
    donors[donorIndex] = {
      ...
      lastDonationDate:
        donations[0]?.donationDate || donors[donorIndex].lastDonationDate,
      updatedAt: new Date().toISOString(),
    };
  ...
  async getAllCampaigns(): Promise<DonationCampaign[]> {
    return JSON.parse(this.getFromStorage(STORAGE_KEYS.CAMPAIGNS, "[]"));
  }
  ...
  async createCampaign(
    ...
    this.setToStorage(STORAGE_KEYS.CAMPAIGNS, JSON.stringify(campaigns));
  ...
  async getAllGoals(): Promise<DonationGoal[]> {
    return JSON.parse(this.getFromStorage(STORAGE_KEYS.GOALS, "[]"));
  }
  ...
  async updateGoalProgress(goalId: string): Promise<void> {
    ...
    this.setToStorage(STORAGE_KEYS.GOALS, JSON.stringify(goals));
  }
  ...
  async getStats(dateRange?: {
    ...
    // Group by type
    const donationsByType = this.groupByType(donations);

    // Group by payment method
    const donationsByPaymentMethod = this.groupByPaymentMethod(donations);

    // Group by source
    const donationsBySource = this.groupBySource(donations);
    ...
    >();
    for (const donation of donations) {
      const current = donorTotals.get(donation.donorId) || {
        amount: 0,
        count: 0,
  ...
  private groupByPaymentMethod(
    ...
    for (const donation of donations) {
      const value = donation.paymentMethod;
      const current = grouped.get(value) || { count: 0, amount: 0 };
      current.count++;
      current.amount += donation.amount;
      grouped.set(value, current);
  ...
  private calculateMonthlyTrends(
    ...
    const trends = new Map<
      string,
      { amount: number; donationCount: number; donors: Set<string> }
    >();
    const now = new Date();
  ...
  async generateFinancialReport(
    ...
    const programAllocations = Array.from(programTotals.entries()).map(
      ([programName, amount]) => ({
        programName,
        amount,
    ...
    const fundraisingExpenses = totalRevenue * 0.1;
    const totalExpenses =
      programExpenses + administrativeExpenses + fundraisingExpenses;
    ...
    return {
      ...
      averageDonation:
        donations.length > 0 ? totalRevenue / donations.length : 0,
      programAllocations,
    };
  }
