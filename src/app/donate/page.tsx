...
export default function DonatePage() {
  ...
  const triggerContainedStripeButton = (container: HTMLDivElement | null) => {
    if (!container) return;
    const stripeButtonWrapper = container.querySelector('[data-automation="stripe-button-recurring"], [data-automation="stripe-button-one-time"]');
  ...
  const handleCardClick = (e: React.MouseEvent<HTMLDivElement>, type: "recurring" | "one-time") => {
    ...
    if (type === "recurring") {
      ...
      if (!customValid) {
        ...
        setTimeout(() => triggerContainedStripeButton(oneTimeCardRef.current), 0);
        return;
      }
  ...
  const handleCardKeyDown = (e: React.KeyboardEvent<HTMLDivElement>, type: "recurring" | "one-time") => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Donations Unavailable</h1>
          <p className="text-gray-600 mb-4">
            Stripe payment processing is not currently configured. Please contact support.
          </p>
        </div>
      </div>
    );
