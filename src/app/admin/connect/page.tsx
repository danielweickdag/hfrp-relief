...
export default function ConnectDashboard() {
  ...
  const loadProducts = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/stripe/connect/products?accountId=${account.id}`);
      const data = await response.json();
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">HFRP Relief | Connect Dashboard</h1>
        <p className="text-muted-foreground">
          Manage Stripe Connect accounts and products for your relief organization
        </p>
      </div>
            ...
            <CardContent>
              ...
              <div className="mt-4">
                <p className="text-sm text-muted-foreground">
                  Email: {account.email} | Created: {formatDate(account.created)}
                </p>
                          ...
                          <CardContent className="pt-6">
                            <div className="flex items-start justify-between">
                              <div className="flex space-x-4">
                                <img
                                  src={product.images[0] || 'https://i.imgur.com/6Mvijcm.png'}
                                  alt={product.name}
                              ...
                              {product.default_price && (
                                <Button
                                  onClick={() => createCheckoutSession(product.default_price!.id)}
                                  size="sm"
                                >
                                  Checkout
                                </Button>
