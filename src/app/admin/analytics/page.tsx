  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-3xl font-bold text-green-600">{formatCurrency(data.totalDonations)}</p>
        <div className="flex items-center mt-2">
          <span className={`inline-block px-2 py-1 text-xs rounded-full ${data.donationGrowth >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {data.donationGrowth >= 0 ? '+' : ''}{data.donationGrowth}%
          </span>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-3xl font-bold text-blue-600">{data.totalVisitors.toLocaleString()}</p>
        <div className="flex items-center mt-2">
          <span className={`inline-block px-2 py-1 text-xs rounded-full ${data.visitorGrowth >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {data.visitorGrowth >= 0 ? '+' : ''}{data.visitorGrowth}%
          </span>
          <span className="text-xs text-gray-500 ml-2">vs previous period</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex items-center mt-2">
          <span className={`inline-block px-2 py-1 text-xs rounded-full ${data.conversionGrowth >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {data.conversionGrowth >= 0 ? '+' : ''}{data.conversionGrowth}%
          </span>
          <span className="text-xs text-gray-500 ml-2">vs previous period</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-2">Avg. Donation</h3>
        <p className="text-3xl font-bold text-orange-600">{formatCurrency(data.avgDonation)}</p>
        <div className="flex items-center mt-2">
          <span className={`inline-block px-2 py-1 text-xs rounded-full ${data.avgDonationGrowth >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {data.avgDonationGrowth >= 0 ? '+' : ''}{data.avgDonationGrowth}%
          </span>
          <span className="text-xs text-gray-500 ml-2">vs previous period</span>
        </div>
      </div>
...
const generateMockData = (timeFilter: string): AnalyticsData => {
  ...
  const donationData = {
    labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].slice(0, factor * 3),
    amounts: Array.from({ length: factor * 3 }, () => Math.floor(Math.random() * 5000) + 1000),
    counts: Array.from({ length: factor * 3 }, () => Math.floor(Math.random() * 30) + 10)
  };

  // Generate traffic data
  const trafficData = {
    labels: Array.from({ length: 7 }, (_, i) => `Day ${i+1}`),
    visits: Array.from({ length: 7 }, () => Math.floor(Math.random() * 200) + 50),
    uniqueVisitors: Array.from({ length: 7 }, () => Math.floor(Math.random() * 150) + 30),
    pageViews: Array.from({ length: 7 }, () => Math.floor(Math.random() * 500) + 100)
  };

  // Generate demographic data
  const demographicData = {
    labels: ['United States', 'Haiti', 'Canada', 'France', 'Dominican Republic', 'Other'],
    data: [45, 25, 10, 8, 7, 5]
  };

  // Generate donation source data
  const donationSourceData = {
    labels: ['Website', 'Mobile App', 'Social Media', 'Email Campaigns', 'Direct Mail'],
    data: [40, 25, 20, 10, 5]
  };

  // Generate top pages data
  const topPagesData = {
    labels: ['/donate', '/programs/education', '/impact', '/gallery', '/contact'],
...
function AnalyticsContent() {
  const [timeFilter, setTimeFilter] = useState(timeFilterOptions[1]); // Default to 30 days
      ...
      fallback={
        <div className="min-h-screen bg-gray-100 p-8">
          <div className="max-w-7xl mx-auto">
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-yellow-700">
                    You do not have permission to view analytics. Contact an administrator for access.
                  </p>
                </div>
              </div>
            </div>
            <Link
              href="/admin"
              className="inline-block bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center"
            >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <p className="text-gray-600">Monitor your website performance and donation metrics</p>
            </div>
            <div className="mt-4 md:mt-0 flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setTimeFilter(timeFilterOptions.find(opt => opt.value === e.target.value) || timeFilterOptions[1])}
                >
              ...
              <Link
                href="/admin"
                className="inline-block bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-lg flex items-center"
              >
                <svg className="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h4 className="font-semibold text-blue-800 mb-2">Analytics Integration Guide</h4>
                <p className="text-blue-700 mb-4">
                  This dashboard is currently displaying mock data for demonstration purposes.
                  In production, you would integrate with real data sources:
                </p>
