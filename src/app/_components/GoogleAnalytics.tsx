  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const trackDonationClick = (element: HTMLElement) => {
        ...
        console.log('📊 Analytics: Donation click tracked', { campaign, amount });
      };
      ...
      const trackGalleryView = (imageId: string, category: string) => {
        ...
        console.log('📊 Analytics: Gallery image view tracked', { imageId, category });
      };
      ...
      const trackDonation = (amount: number, currency = 'USD', campaign = 'general') => {
        gtag('event', 'purchase', {
        ...
        console.log('📊 Analytics: Donation completed tracked', { amount, currency, campaign });
      };
      ...
      document.addEventListener('click', (e) => {
        ...
        if (target.closest('[data-track="donate"]')) {
          const donateButton = target.closest('[data-track="donate"]') as HTMLElement;
          trackDonationClick(donateButton);
        }

        // Track gallery image clicks
        if (target.closest('[data-track="gallery-image"]')) {
          const imageElement = target.closest('[data-track="gallery-image"]') as HTMLElement;
          const imageId = imageElement.getAttribute('data-image-id') || 'unknown';
          const category = imageElement.getAttribute('data-category') || 'unknown';
