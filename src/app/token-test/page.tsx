...
export default function TokenTestPage() {
  ...
  const [tokenInfo, setTokenInfo] = useState<{ exp: number; iat: number; stream: string; host: string } | null>(null);
    ...
    try {
      ...
      if (location) {
        ...
        if (tokenMatch) {
          ...
          if (parts.length === 3) {
            const payload = JSON.parse(atob(parts[1])) as { exp: number; iat: number; stream: string; host: string };
    ...
    <div className="container mx-auto p-8">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          {tokenInfo ? (
            <div className="space-y-2 text-sm">
              <p><strong>Stream:</strong> {tokenInfo.stream}</p>
              <p><strong>Host:</strong> {tokenInfo.host}</p>
              ...
              <p><strong>Lifetime:</strong> {tokenInfo.exp - tokenInfo.iat} seconds</p>
            </div>
          ) : (
      ...
      <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <ul className="text-yellow-700 space-y-1">
          <li>• This page automatically tests token refresh every 30 seconds</li>
          <li>• Tokens expire after 60 seconds</li>
          <li>• Watch the logs to see token refresh in action</li>
          <li>• The radio player should now handle token expiration automatically</li>
        </ul>
      </div>
    </div>
  );
