...
export default function StripeAdminPage() {
  ...
  const [status, setStatus] = useState<SyncStatus | { error: string } | null>(
    null,
  );
  ...
  async function loadStatus() {
    ...
    try {
      ...
      const data: { success: boolean; data?: SyncStatus; error?: string } =
        await res.json();
      if (data.success) {
