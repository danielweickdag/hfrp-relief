...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!customerId || !priceId) {
      return NextResponse.json({ error: 'customerId and priceId are required' }, { status: 400 });
    }

    const subParams: Stripe.SubscriptionCreateParams = {
      customer: customerId,
      items: [{ price: priceId, quantity: quantity && quantity > 0 ? quantity : 1 }],
      payment_behavior: 'default_incomplete',
      expand: ['latest_invoice.payment_intent'],
      ...(automaticTaxEnabled ?? true ? { automatic_tax: { enabled: true } } : {}),
      payment_settings: {
        save_default_payment_method: saveDefaultPaymentMethod === 'on_subscription' ? 'on_subscription' : undefined,
      },
      ...(metadata ? { metadata } : {}),
    };
    ...
    if (latestInvoiceRaw && typeof latestInvoiceRaw !== 'string') {
      ...
      if (pi && typeof pi !== 'string') {
        ...
        const piObj = await stripe.paymentIntents.retrieve(pi, { stripeAccount: accountIdFinal });
