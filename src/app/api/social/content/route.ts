...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<SocialContentResponse>> {
  try {
...
async function generateAISocialContent(params: {
  ...
  const contentTemplates: ContentTemplate[] = [
    {
      ...
      mediaType: "photo",
      callToAction:
        "Donate today to help more families find home! Link in bio.",
    },
    {
      platform: "Instagram",
      ...
      mediaType: "carousel",
      callToAction:
        "Swipe to see more amazing students! Donate via link in bio.",
    },
    {
      platform: "Twitter",
    ...
    },
    {
      ...
      callToAction:
        "Follow for more behind-the-scenes content! Link in bio to help.",
    },
  ];
  ...
  if (platform && platform !== "all") {
    filteredContent = contentTemplates.filter(
...
async function scheduleAutomatedPosts(content: ContentTemplate[]) {
  ...
  const totalEstimatedReach = content.reduce(
