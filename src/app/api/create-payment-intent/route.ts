...
export async function POST(request: NextRequest) {
  try {
    ...
    const cents = typeof amountCents === 'number' && Number.isFinite(amountCents)
    ...
    if (cents <= 0) {
      return NextResponse.json({ error: 'amount or amountCents must be a positive value' }, { status: 400 });
    }
    ...
    // Derive whether automatic tax was applied without using any casts
    const automaticTaxEnabledApplied = !!createParams.automatic_tax?.enabled;
...
export async function GET(_request: NextRequest) {
  return NextResponse.json({
    status: 'ok',
    modeFromKey: process.env.STRIPE_SECRET_KEY?.startsWith('sk_live_') ? 'live' : 'test',
    accountDefault:
      process.env.STRIPE_CONNECTED_ACCOUNT_ID ||
      process.env.NEXT_PUBLIC_STRIPE_CONNECTED_ACCOUNT_ID ||
      null,
