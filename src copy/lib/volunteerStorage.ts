...
const DEFAULT_PROGRAMS: VolunteerProgram[] = [
  ...
  },
  {
    ...
    description: 'Support our mobile healthcare clinics and health education programs',
    category: 'Healthcare',
...
const DEFAULT_TRAININGS: VolunteerTraining[] = [
  {
    ...
    description: 'Introduction to HFRP mission, values, and volunteer guidelines',
    duration: 2,
    type: 'in_person',
    isRequired: true,
    createdAt: new Date().toISOString(),
...
class VolunteerStorageService {
  ...
  private initializeDefaults() {
    ...
    if (!localStorage.getItem(STORAGE_KEYS.TRAININGS)) {
      localStorage.setItem(STORAGE_KEYS.TRAININGS, JSON.stringify(DEFAULT_TRAININGS));
    }
  }

  // Volunteer CRUD operations
  async getAllVolunteers(filters?: VolunteerFilters): Promise<Volunteer[]> {
    const volunteers = JSON.parse(localStorage.getItem(STORAGE_KEYS.VOLUNTEERS) || '[]') as Volunteer[];
    ...
    if (filters) {
      ...
      if (filters.availability?.length) {
        filtered = filtered.filter(v =>
          v.availability.some(avail => filters.availability?.includes(avail.dayOfWeek))
        );
      }

      if (filters.hasTransportation !== undefined) {
        filtered = filtered.filter(v => v.hasTransportation === filters.hasTransportation);
      }

      if (filters.backgroundCheckCompleted !== undefined) {
        filtered = filtered.filter(v => v.backgroundCheckCompleted === filters.backgroundCheckCompleted);
      }
      ...
      filtered.sort((a, b) => {
        ...
        switch (sortBy) {
          ...
          case 'totalHours':
            ...
            break;
          case 'lastActive':
            comparison = new Date(b.lastActiveDate || 0).getTime() - new Date(a.lastActiveDate || 0).getTime();
            break;
        }
  ...
  async createVolunteer(data: Omit<Volunteer, 'id' | 'createdAt' | 'updatedAt' | 'totalHours' | 'totalShifts'>): Promise<Volunteer> {
    const volunteers = await this.getAllVolunteers();
  ...
  async updateVolunteer(id: string, data: Partial<Volunteer>): Promise<Volunteer | null> {
    const volunteers = await this.getAllVolunteers();
  ...
  async getAllShifts(filters?: VolunteerScheduleFilters): Promise<VolunteerShift[]> {
    const shifts = JSON.parse(localStorage.getItem(STORAGE_KEYS.SHIFTS) || '[]') as VolunteerShift[];

    let filtered = [...shifts];

    if (filters) {
      if (filters.volunteerId) {
        filtered = filtered.filter(s => s.volunteerId === filters.volunteerId);
      }
  ...
  async createShift(data: Omit<VolunteerShift, 'id' | 'createdAt' | 'updatedAt'>): Promise<VolunteerShift> {
    const shifts = await this.getAllShifts();
    ...
    const newShift: VolunteerShift = {
      ...data,
      id: `shift-${Date.now()}`,
      volunteerName: volunteer ? `${volunteer.firstName} ${volunteer.lastName}` : 'Unknown',
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
  ...
  async updateShift(id: string, data: Partial<VolunteerShift>): Promise<VolunteerShift | null> {
    const shifts = await this.getAllShifts();
  ...
  private async updateVolunteerStats(volunteerId: string, hours: number, shifts: number) {
  ...
  async checkOutShift(shiftId: string, actualHours?: number): Promise<VolunteerShift | null> {
    const shift = await this.getShiftById(shiftId);
    if (!shift || !shift.checkInTime) return null;

    const checkOutTime = new Date().toISOString();
    const hours = actualHours || this.calculateHours(shift.checkInTime, checkOutTime);
  ...
  async createProgram(data: Omit<VolunteerProgram, 'id' | 'createdAt' | 'updatedAt'>): Promise<VolunteerProgram> {
    const programs = await this.getAllPrograms();
  ...
  async createTraining(data: Omit<VolunteerTraining, 'id' | 'createdAt' | 'updatedAt'>): Promise<VolunteerTraining> {
    const trainings = await this.getAllTrainings();
  ...
  async getVolunteerStats(): Promise<VolunteerStats> {
    ...
    const shiftsThisYear = shifts.filter(s => {
      const shiftDate = new Date(s.date);
      return shiftDate.getFullYear() === currentYear && s.status === 'completed';
    });

    const totalHoursThisMonth = shiftsThisMonth.reduce((sum, s) => sum + (s.actualHours || s.duration), 0);
    const totalHoursThisYear = shiftsThisYear.reduce((sum, s) => sum + (s.actualHours || s.duration), 0);
    ...
    const programStats: Record<string, { volunteers: Set<string>, hours: number }> = {};
    ...
    for (const shift of shifts.filter(s => s.status === 'completed')) {
      ...
      programStats[shift.programId].hours += shift.actualHours || shift.duration;
    }
    ...
    return {
      ...
      totalHoursThisMonth,
      totalHoursThisYear,
      averageHoursPerVolunteer: activeVolunteers.length > 0
  ...
  async generateHoursReport(volunteerId: string, startDate: string, endDate: string): Promise<VolunteerHoursReport | null> {
    const volunteer = await this.getVolunteerById(volunteerId);
    ...
    for (const shift of shifts) {
      ...
      switch (shift.status) {
        ...
        case 'completed':
          ...
          programHours[shift.programName].hours += shift.actualHours || shift.duration;
    ...
    const totalHours = Object.values(programHours).reduce((sum, p) => sum + p.hours, 0);
    const totalShifts = Object.values(programHours).reduce((sum, p) => sum + p.shifts, 0);
  ...
  // Bulk operations
  async bulkUpdateVolunteerStatus(volunteerIds: string[], status: VolunteerStatus): Promise<void> {
    const volunteers = await this.getAllVolunteers();
  ...
  async assignTrainingToVolunteers(volunteerIds: string[], trainingId: string): Promise<void> {
    const volunteers = await this.getAllVolunteers();

    for (const volunteer of volunteers) {
      if (volunteerIds.includes(volunteer.id) && !volunteer.trainingsCompleted.includes(trainingId)) {
