...
export function useSocialMedia() {
  const shareStory = (story: { title: string; description: string; url?: string }) => {
    const shareUrl = story.url || (typeof window !== 'undefined' ? window.location.href : '');

    return {
      facebook: () => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`, '_blank'),
      twitter: () => window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(story.title)}&hashtags=HaitianFamilyRelief,Hope4Haiti`, '_blank'),
      instagram: () => window.open('https://instagram.com/haitianfamilyreliefproject', '_blank'),
      whatsapp: () => window.open(`https://wa.me/?text=${encodeURIComponent(`${story.title} ${shareUrl}`)}`, '_blank')
    };
  };

  const followUs = {
    facebook: () => window.open('https://facebook.com/haitianfamilyreliefproject', '_blank'),
