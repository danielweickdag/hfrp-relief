...
const commonTags = {
  feeding: ['community-kitchen', 'food-prep', 'volunteers', 'distribution', 'fresh-food', 'vegetables', 'family-meals', 'nutrition', 'community', 'gathering', 'support', 'training', 'daily-meals', 'service', 'children', 'health', 'kitchen', 'operations', 'supplies', 'equipment', 'food-security', 'access', 'outreach', 'families', 'programs'],
  healthcare: ['medical-care', 'health-education', 'community', 'mobile-clinic', 'patient-care', 'preventive-care', 'health-awareness', 'nutrition', 'hygiene', 'disease-prevention', 'checkup', 'treatment', 'healthcare-workers', 'clinic'],
  education: ['school-supplies', 'resources', 'children', 'education', 'learning', 'teaching', 'classroom', 'students', 'books', 'uniforms', 'tuition', 'literacy', 'training', 'workshops', 'technology', 'computer'],
  housing: ['housing-assistance', 'shelter', 'families', 'construction', 'repair', 'emergency-shelter', 'home-building', 'infrastructure', 'community-development', 'disaster-response', 'safe-housing', 'construction-training']
};
...
export default function PhotoUpload() {
      ...
      if (file.type.startsWith('image/')) {
        ...
        reader.onload = (e) => {
          ...
          const newPhoto: UploadedPhoto = {
            ...
            preview,
            ...currentPhoto,
            title: currentPhoto.title || file.name.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' '),
          };
  ...
  // Add custom tag
  const addCustomTag = () => {
    if (newTag.trim() && !currentPhoto.tags.includes(newTag.trim().toLowerCase())) {
  ...
  const updatePhotoMetadata = (photoId: string, field: keyof PhotoUploadData, value: string) => {
  ...
  const selectedCategoryColor = programCategories.find(cat => cat.id === currentPhoto.category)?.color || 'blue';

  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">📸 Photo Upload Manager</h1>
        <p className="text-gray-600">Upload and manage photos for HFRP program galleries</p>
      </div>

      {/* Default Metadata Form */}
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Default Photo Metadata</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Program Category</label>
            <select
              value={currentPhoto.category}
              onChange={(e) => setCurrentPhoto(prev => ({
                ...prev,
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Default Title</label>
            <input
              type="text"
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Default Description</label>
            <textarea
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {currentPhoto.tags.map(tag => (
                <span key={tag} className={`px-3 py-1 bg-${selectedCategoryColor}-100 text-${selectedCategoryColor}-800 rounded-full text-sm flex items-center gap-1`}>
                  #{tag}
                  <button
            ...
            {/* Common Tags */}
            <div className="mb-3">
              <p className="text-sm text-gray-600 mb-2">Common tags for {currentPhoto.category}:</p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!currentPhoto.tags.includes(tag)) {
                        setCurrentPhoto(prev => ({ ...prev, tags: [...prev.tags, tag] }));
                      }
                    }}
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 space-y-3">
                    <input
                      type="text"
                      value={photo.title}
                      onChange={(e) => updatePhotoMetadata(photo.id, 'title', e.target.value)}
                    ...
                    <div className="flex items-center justify-between">
                      <select
                        ...
                        onChange={(e) => updatePhotoMetadata(photo.id, 'category', e.target.value)}
